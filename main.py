...
# Connection primitives live in db.py. init_db() (schema + migrations) stays
# here because it wires up many module-level globals in this file.
from db import (
    DATABASE_URL,
    USE_POSTGRES,
    PgConnectionWrapper,
...
CREATE TABLE IF NOT EXISTS usage_log (
            ...
            # SQLite migrations — add columns that may not exist in older schemas
            sqlite_migrations = [
                ("users", "telegram_chat_id", "TEXT DEFAULT ''"),
                ("users", "anthropic_key", "TEXT DEFAULT ''"),
            ]
            for table, col, col_type in sqlite_migrations:
                try:
                    conn.execute(f"ALTER TABLE {table} ADD COLUMN {col} {col_type}")
                    conn.commit()
...
# Slack outbound helpers live in slack.py. (They had been defined three times
# here; the last plain-text set was the effective one — preserved in slack.py.)
from slack import send_slack_message, send_slack_agent_result, send_slack_daemon_alert
...
# get_api_key lives in deps.py so route modules can share it without importing main.
...
async def demo_deploy(request: Request):
    ...
    if not task:
        raise HTTPException(status_code=400, detail="task_description required")
    # Use platform key for demo runs
    import uuid
    agent_id = str(uuid.uuid4())
    now = datetime.now(timezone.utc).isoformat()
    conn = get_db()
    try:
        db_execute(conn, 
            "INSERT INTO agents (id, type, name, status, task, result, created_at, updated_at, user_api_key) VALUES (?,?,?,?,?,?,?,?,?)",
            (agent_id, agent_type, agent_type, "running", task, None, now, now, "demo")
        )
        conn.commit()
    finally:
        conn.close()
    asyncio.create_task(execute_task(
        agent_id=agent_id, agent_type=agent_type, task_description=task,
        model=None, user_api_key="demo", stream=False
    ))
    return {"agent_id": agent_id, "status": "running", 
            "message": f"Poll /api/v1/status/{agent_id} for results."}
